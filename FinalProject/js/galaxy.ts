...
function displayGalaxy() {
  let galaxyName = (<HTMLInputElement>document.getElementById('galaxyInput')).value;

  //Displays text including the text the user entered
  if (galaxyName !== "") {
      (<HTMLInputElement>document.getElementById("outputText")).textContent = `Displaying pets from the ${galaxyName} Galaxy branch.`;
